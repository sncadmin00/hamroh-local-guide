...
// ---- upsertTour (delegates to shared core in upsert-tour.server) ----

export const upsertTour = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => upsertTourInputSchema.parse(input))
  .handler(async ({ context, data }) => {
    const { userId } = context;
    const tour = await upsertTourCore(data, userId);
    return { ok: true, id: tour.id as string, tour };
  });
