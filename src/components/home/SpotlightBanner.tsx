        ...
        {items.map((s) => {
          ...
          return (
            <Link
              ...
              to="/spotlight/$id"
              params={{ id: s.id }}
              className="group flex w-[85%] sm:w-full shrink-0 snap-start items-center gap-4 border-y border-slate-900/10 py-4 transition-opacity hover:opacity-70"
            >
      ...
      {items.length > 1 && (
        <div className="mt-2 flex justify-center gap-1.5">
          {items.map((s) => (
            <span
              key={s.id}
              className="h-1 w-6 rounded-full bg-slate-900/15"
            />
          ))}
        </div>
      )}
